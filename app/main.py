...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["https://intellinote-forge.vercel.app","http://localhost:5173"],
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://intellinote-forge.vercel.app",
        "http://localhost:5173",
        "http://localhost:3000",
        "http://127.0.0.1:5173",
        "http://127.0.0.1:3000",
...
# Define in-memory storage classes first
            ...
            if match:
                if "$push" in update:
                    for push_key, push_val in update["$push"].items():
                        if push_key not in item:
                            item[push_key] = []
                        item[push_key].append(push_val)
                if "$setOnInsert" in update:
                    for set_key, set_val in update["$setOnInsert"].items():
                        if set_key not in item:
...
class InMemoryDatabase:
    ...
    def __init__(self):
        ...
        self.responses = InMemoryCollection()
...
async def process_with_groq_ai(text: str, prompt_type: str = "summarize") -> str:
    if not groq_client:
        raise HTTPException(
            ...
            detail="AI service is not configured. Please check GROQ_API_KEY."
        )
...
async def extract_key_points_from_summary(summary: str) -> List[str]:
    if not groq_client:
        return ["Key points extraction not available - AI service not configured"]
...
async def root():
    return {
        ...
        "version": "1.0.0",
        "pdf_parser": PDF_PARSER,
        "database_status": "in-memory (development mode)" if USE_MOCK_DB else "connected to Atlas",
...
async def login(login_data: UserLogin):
    ...
    try:
        ...
        print(f"User logged in successfully: {login_data.email}")
...
async def get_user_documents(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        docs_list = await documents.to_list(length=100)
...
async def get_user_responses(
    ...
    try:
        ...
        print(f"Get responses error: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def health_check():
    """Health check endpoint"""
    mongo_status = "in-memory (development)" if USE_MOCK_DB else "connected to Atlas"
    if not USE_MOCK_DB:
        try:
            await client.admin.command('ping')
            mongo_status = "connected"
    ...
    return {
        ...
        "timestamp": datetime.utcnow().isoformat(),
        "pdf_parser": PDF_PARSER,
        "database_type": "in-memory" if USE_MOCK_DB else "Atlas",
        "services": {
            "mongodb": mongo_status,
            "groq_ai": groq_status
        }
...
async def hit_api_periodically():
...
async def startup_event():
    asyncio.create_task(hit_api_periodically())